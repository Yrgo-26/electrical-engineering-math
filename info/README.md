# Kursinfo – Elteknisk matematik

## Lärare
Erik Pihl ([erik.axel.pihl@gmail.com](mailto:erik.axel.pihl@gmail.com))

---

## Schema

### Ee26

| Vecka | Datum | Lektion | Ämne |
|----|-------|-----|-----------------------------------------------|
| 36 | 1/9   | L01 | Kursintroduktion och talsystem                |
| 37 | 8/9   | L02 | Algebra                                       |
| 38 | 15/9  | L03 | Linjära ekvationer och olikheter              |
| 39 | 22/9  | L04 | Ekvationssystem                               |
| 40 | 29/9  | L05 | Potenser och rötter                           |
| 41 | 6/10  | L06 | Andragradsekvationer                          |
| 42 | 13/10 | L07 | Vektorer – **Dugga 1**                        |
| 42 | 15/10 | L08 | Funktioner                                    |
| 43 | 20/10 | L09 | Trigonometriska funktioner                    |
| 43 | 22/10 | L10 | Exponentialfunktioner, logaritmer och decibel |
| 44 | 27/10 | L11 | Repetition och övningsdugga                   |
| 44 | 29/10 | L12 | Derivata (del I) – övningsdugga 2             |
| 45 | 3/11  | L13 | Derivata (del II) – **Dugga 2**               |
| 45 | 5/11  | L14 | Integraler                                    |
| 46 | 10/11 | L15 | Komplexa tal (del I)                          |
| 46 | 12/11 | L16 | Komplexa tal (del II)                         |
| 47 | 17/11 | L17 | Komplexa tal (del III)                        |
| 48 | 24/11 | L18 | **Laboration** – Sinusrekonstruktion med DAC  |
| 49 | 1/12  | L19 | Repetition och tentamensförberedelse          |
| 50 | 8/12  | L20 | **Tentamen**                                  |


### Eh26

| Vecka | Datum | Lektion | Ämne |
|----|-------|-----|-----------------------------------------------|
| 36 | 3/9   | L01 | Kursintroduktion och talsystem                |
| 37 | 10/9  | L02 | Algebra                                       |
| 38 | 17/9  | L03 | Linjära ekvationer och olikheter              |
| 39 | 24/9  | L04 | Ekvationssystem                               |
| 40 | 1/10  | L05 | Potenser och rötter                           |
| 41 | 8/10  | L06 | Andragradsekvationer                          |
| 42 | 13/10 | L07 | Vektorer – **Dugga 1**                        |
| 42 | 15/10 | L08 | Funktioner                                    |
| 43 | 20/10 | L09 | Trigonometriska funktioner                    |
| 43 | 22/10 | L10 | Exponentialfunktioner, logaritmer och decibel |
| 44 | 27/10 | L11 | Repetition och övningsdugga                   |
| 44 | 29/10 | L12 | Derivata (del I) – övningsdugga 2             |
| 45 | 3/11  | L13 | Derivata (del II) – **Dugga 2**               |
| 45 | 5/11  | L14 | Integraler                                    |
| 46 | 10/11 | L15 | Komplexa tal (del I)                          |
| 46 | 12/11 | L16 | Komplexa tal (del II)                         |
| 47 | 19/11 | L17 | Komplexa tal (del III)                        |
| 48 | 26/11 | L18 | **Laboration** – Sinusrekonstruktion med DAC  |
| 49 | 3/12  | L19 | Repetition och tentamensförberedelse          |
| 50 | 10/12 | L20 | **Tentamen**                                  |

---

## Examination och betygsnivåer

### Upplägg
* Två duggor (**Dugga 1–2**) under kursens gång.
* En laboration (**L18**).
* En skriftlig tentamen som kursavslut (**L20**).

### Poängfördelning
* Dugga 1–2 ger upp till **1,0 bonuspoäng** totalt (0,5 p per dugga, kräver minst 3,5/5 p).
* Den skriftliga tentamen ger upp till **25 poäng**.

### Betygsgränser

| Betyg | Gräns |
|-------|-------|
| IG | < 10,0 p |
| G | ≥ 12,5 p |
| VG | ≥ 17,5 p |

För godkänt betyg krävs att alla kursens lärandemål är uppnådda.

---

## Kursmaterial

### Litteratur
Kursmaterialet finns samlat i detta repo under [lectures/](../lectures/). Varje lektion innehåller:
* **Bilaga A** – kursmaterial och typexempel.
* **Bilaga B** – lektionsuppgifter.
* **Bilaga C** – lösningsförslag.

### Hjälpmedel
* Valfri miniräknare (används under hela kursen).
* På tentamen tillåts dessutom ett eget A4-ark med formler och ett A4-ark med egna anteckningar (båda sidor).

---
